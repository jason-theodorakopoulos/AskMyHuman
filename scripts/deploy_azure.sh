...
deployment_environment() {
  require_variables AZURE_LOCATION POSTGRES_ADMIN_PASSWORD ACS_SOURCE_PHONE_NUMBER \
    ...
    PROVISION_LIVE_DATABASE USE_LIVE_DATABASE
  [[ "$PROVISION_LIVE_DATABASE" =~ ^(true|false)$ ]] || fail 'PROVISION_LIVE_DATABASE must be true or false'
  [[ "$USE_LIVE_DATABASE" =~ ^(true|false)$ ]] || fail 'USE_LIVE_DATABASE must be true or false'
  [[ "$USE_LIVE_DATABASE" != true || "$PROVISION_LIVE_DATABASE" == true ]] || fail 'live database use requires provisioning'
  ...
  PARAMETERS_SHA256="$(jq -n '[env | {AZURE_LOCATION, CONTAINER_REGISTRY_SERVER,
    CONTAINER_REGISTRY_RESOURCE_ID, POSTGRES_ADMIN_PASSWORD, ACS_SOURCE_PHONE_NUMBER,
    ENTRA_TENANT_ID, ENTRA_CLIENT_ID, ENTRA_CLIENT_SECRET,
    AUTHORIZED_AGENT_APP_IDS, EXISTING_ACS_RESOURCE_ID, ACS_CALLBACK_AUDIENCE,
    ACS_CALLBACK_URL, MCP_ALLOWED_HOSTS, PROVISION_LIVE_DATABASE,
    USE_LIVE_DATABASE}]' | hash_json)"
}
