...
from ask_my_human.application.ports import Admission
    ...
    async def attach_call_id(self, request_id: UUID, call_id: str) -> bool:
        async with self._pool.connection() as connection:
            cursor = await connection.execute(
                ...
                WHERE request_id = %s
                  AND status = 'pending'
                  AND acs_call_connection_id IS NULL
                """,
                (call_id, request_id),
            )
    ...
    async def complete_if_pending(self, result: AskHumanResult) -> bool:
        async with self._pool.connection() as connection:
            cursor = await connection.execute(
                ...
                """,
                (result.status.value, result.outcome.value, result.answer, result.request_id),
            )
            return int(cursor.rowcount) == 1
